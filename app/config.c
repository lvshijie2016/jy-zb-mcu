...
void get_gpio(uint8_t gpio, uint16_t pin, uint8_t function,uint8_t I_O, uint8_t HL ,uint8_t edge)
{
	...
	if(function&0xF0) function = 0;  //Ãÿ ‚π¶ƒ‹IO
	if(!function)
	{
		if(I_O == IO_Output)
		{
...
//	ADC_EnableConversionInt(ADC_CHN6_ENABLE);
 ...
 // GPIO_Configuration();
    ...
    ADC_InitStructure.ADC_ExternalTrigConv = ADC_ExternalTrigConv_T1_CC1;//ADC1 Õ®µ¿1£¨
...
static void timer0_init_t(void)
{
  ...
  NVIC_InitTypeDef NVIC_StructInit;
	...
	//1ms for 1 tick
	TIM_StructInit.TIM_Period=100;                                                                      //ARRºƒ¥Ê∆˜÷µ
	TIM_StructInit.TIM_Prescaler=SystemCoreClock/100000;                                                //‘§∑÷∆µ÷µ
 
	
	TIM_StructInit.TIM_ClockDivision=TIM_CKD_DIV1;                                                      //≤…—˘∑÷∆µ÷µ
	TIM_StructInit.TIM_CounterMode=TIM_CounterMode_Up;                                                     //º∆ ˝ƒ£ Ω
...
static void Tim2_Init(u16 Prescaler,u16 Period)
{
    ...
    /* πƒ‹TIM1 ±÷”,ƒ¨»œ ±÷”‘¥Œ™PCLK1(PCLK1Œ¥∑÷∆µ ±≤ª±∂∆µ,∑Ò‘Ú”…PCLK1±∂∆µ ‰≥ˆ),ø…—°∆‰À¸ ±÷”‘¥*/
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2, ENABLE);
       
    TIM_StructInit.TIM_Period=Period;                                                                      //ARRºƒ¥Ê∆˜÷µ
    TIM_StructInit.TIM_Prescaler=Prescaler;                                                                //‘§∑÷∆µ÷µ
    /* ˝◊÷¬À≤®∆˜≤…—˘∆µ¬ ,≤ª”∞œÏ∂® ±∆˜ ±÷”*/
    TIM_StructInit.TIM_ClockDivision=TIM_CKD_DIV1;                                                         //≤…—˘∑÷∆µ÷µ
    TIM_StructInit.TIM_CounterMode=TIM_CounterMode_Up;                                                     //º∆ ˝ƒ£ Ω
    TIM_StructInit.TIM_RepetitionCounter=0;
...
void exit_irq_init()
{
    ...
    // π”√Õ‚≤ø÷–∂œ∑Ω Ω
    SYSCFG_EXTILineConfig(GPIO_PortSourceGPIOA, GPIO_PinSource0|GPIO_PinSource5|GPIO_PinSource11|GPIO_PinSource12);	//÷–∂œœﬂ0¡¨Ω”GPIOA.0  GPIOA.5  

    EXTI_InitStructure.EXTI_Line = EXTI_Line0|EXTI_Line5|EXTI_Line11;	//…Ë÷√∞¥º¸À˘”–µƒÕ‚≤øœﬂ¬∑   
    ...
    EXTI_InitStructure.EXTI_Line = EXTI_Line12;	//…Ë÷√∞¥º¸À˘”–µƒÕ‚≤øœﬂ¬∑   
    EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Rising;  //…œ…˝/œ¬Ωµ—ÿ¥•∑¢
    EXTI_Init(&EXTI_InitStructure);	// ≥ı ºªØÕ‚≤ø÷–∂œ
